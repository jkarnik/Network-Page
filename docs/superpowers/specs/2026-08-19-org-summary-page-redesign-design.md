...
- Visibility-rooted clusters are visually distinct from network-rooted clusters — different treatment, same computation.
...
- **No monitoring-health widgets.** API polling counts, rate-limit headroom, agent fleet and agent host health all live on the Monitor Health page. This page shows only per-panel source scope, and links out.
