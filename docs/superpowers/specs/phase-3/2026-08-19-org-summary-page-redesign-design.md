...
One dependency this creates: the backup figure below is only as complete as the alerting configuration. An integration that does not raise a failover incident contributes zero to it, and the count would understate reality without saying so. This is the same shape of dependency as the *entity stopped reporting* condition in the Monitor Health spec — it belongs in alerting configuration, not in dashboard inference.

Usefully, it is also **observable rather than merely assumed**: band 3's WAN path posture computes backup state fleet-wide and independently of alerting, so a disagreement between the two is diagnostic. Five sites on backup in band 3 against two failover incidents in band 1 means three integrations are not alerting on failover. The two widgets validate each other, which is why the fleet-wide one is worth having even though band 1 carries the headline.
...
- **Sites on a backup path** — count of sites with an open failover-class incident. Expressed as an *incident* count rather than as observed path state, so it stays consistent with the rest of the band: band 1 is the **actionable** set, meaning things someone has been alerted about. Observed path state is fleet-wide too (band 3 computes it from topology), so the choice here is about what band 1 means, not about coverage — and the two figures deliberately cross-check each other, as below. Detailed path state lives in band 3.
...
- **Redundancy verification** — per site, whether dual gateways or dual uplinks are genuinely independent paths, **computed from adjacency** rather than asserted from device count. Fleet-wide, since topology is. Replaces the site page's guessed "2/2 paths available" indicator with a real answer.
- **Topology coverage** — sites where the engine knows devices that have no telemetry source at all. Because topology is fleet-wide it is the **authoritative inventory**, which makes the gap between topology-known devices and telemetry-covered devices exactly measurable rather than estimated. These are the devices most at risk of being read as healthy, and this is the only place they are visible on this page.
- **WAN path posture** — per site, which uplink is carrying traffic and whether that is the intended primary. Counted in *sites*, not percentages, so it stays readable at any fleet size. **Successor to the removed WAN Resilience donut** (§6.5), and the reason failover visibility is not lost in the redesign. **Fleet-wide**, because both halves of the question are answerable fleet-wide:
  - **Intended primary** comes from the topology engine, which is fleet-wide — so uplink role does not depend on any integration exposing it.
  - **Current link state** for every uplink port comes from port nodes (§2.3), and interface state is one of the few genuinely universal signals across API and agent collection.
...
| WAN path posture (band 3) | **A** | Fleet-wide: topology gives intended primary, port nodes give link state |
...
6. ~~**Does the topology engine record uplink role or priority?**~~ **Resolved: topology is fleet-wide**, so uplink role comes from the engine for every site and WAN path posture is fleet-wide rather than source-scoped. The residual question is narrower and is a data-model detail rather than a blocker: whether the engine distinguishes *intended primary* from merely *first-listed* uplink on sites with more than two uplinks.
