...
4. ~~**Where does Monitor Health sit in the nav?**~~ **Resolved:** added as a fourth top-level item, so the order is Summary / Sites / Devices / Monitor Health. A placeholder `monitor-health.html` renders the shared header and the planned scope, so the nav link resolves rather than 404ing until the page is built.
